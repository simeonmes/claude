...
class BotBrain {
  ...
  decide(car, game) {
    ...
    const tp = ball.frozen ? 0 : clamp(d / 1500, 0, 0.9) * L.pred;
    ...
    const danger = ball.x * -atk > A.W * 0.3 && ball.vx * -atk > 250;
  ...
  drive(car, game, out) {
    ...
    const sT = nearWall ? pathNearest(A, this.tx, this.ty).s : floorS(A, clamp(this.tx, -A.W + 30, A.W - 30));
    ...
    if (hopping && Math.abs(car.ds) > 850) want = 0;
    ...
    if (hopping && this.cool <= 0 && closing > 120 && Math.abs(ball.x - car.x) < R + 48 + closing * 0.3) {
    ...
    const reach = Math.abs(car.ds) * 0.14;
    ...
    if (L.aerial && ahead > R * 0.3 && hUp > 330 && ahead < 300 && car.boost > 45 && ball.vy > -350) {
      this.holdJump = P.JUMP_HOLD_T + 0.02;
      this.airPlan = "aerial";
      this.cool = 0.6;
  ...
  fly(car, game, out) {
    ...
    if (this.airPlan === "hop") {
      ...
      out.sy = 0;
      if (car.flipAvailable && !this.pending && car.airTime > 0.16) {
        this.holdJump = 0;
    ...
    if (this.airPlan && d < 750 && this.aimBall && goalSide) {
      ...
      if (car.flipAvailable && d < R + 78 && car.airTime > 0.1 && !this.pending) {
        ...
        const ahead = rx * atk > 18 + Math.max(0, car.vx * atk) * 0.03;
