...
const UI = {
  prefs: { brawler: "buck", map: "canyon", level: "normal", muted: false },
  ...
  load() {
    ...
    if (!MAPS[this.prefs.map]) this.prefs.map = "canyon";
  ...
  refresh() {
    ...
    for (const b of document.querySelectorAll('.chips[data-group="map"] button')) b.classList.toggle("on", b.dataset.value === p.map);
  ...
  results() {
    ...
    this.$("overTitle").textContent = won ? "VICTORY!" : "DEFEAT";
    this.$("overTitle").className = won ? "win" : "lose";
    this.$("overScore").innerHTML = `<span class="b">${G.teamGems[0]}</span> <small>gems</small> <span class="r">${G.teamGems[1]}</span>`;
    const rows = G.brawlers.slice().sort((a, b) => a.team - b.team || b.stats.kills - a.stats.kills);
    this.$("overStats").innerHTML = "<tr><th></th><th>Brawler</th><th>Defeats</th><th>Deaths</th><th>Gems</th><th>Damage</th></tr>" +
      rows.map((b) => `<tr class="t${b.team}${b.isPlayer ? " me" : ""}"><td>${b.name}</td><td>${b.def.name}</td><td>${b.stats.kills}</td><td>${b.stats.deaths}</td><td>${b.stats.gems}</td><td>${b.stats.dmg.toLocaleString()}</td></tr>`).join("");
    this.show("over");
  },
};
...
function playerInput() {
  ...
  const def = p.def;
  const auto = (kind) => autoAim(p, kind === "sup" ? def.superAim : def.aim, def.shotSpeed);
  ...
  if (ta) {
    const shape = ta.kind === "sup" ? def.superAim : def.aim;
  ...
  if (pad) {
    ...
    const aimed = rm > 0.3 ? { ang: Math.atan2(pad.ry, pad.rx), dist: rm * def.aim.range } : null;
    if (aimed) R.aim = { kind: pad.super ? "sup" : "atk", ang: aimed.ang, dist: aimed.dist, strong: true };
    if (pad.fire) doFire(aimed || auto("atk"));
    if (Input.padEdge(pad, "super")) doSuper(aimed ? { ang: aimed.ang, dist: rm * def.superAim.range } : auto("sup"));
