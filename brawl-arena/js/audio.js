...
const Sound = (() => {
  ...
  const bank = {
    ...
    kick: (v) => { noise(0.1, 0.5 * v, 700, "lowpass"); tone(160, 0.1, "sine", 0.25 * v, 0, 70); },
    bounce: (v) => { tone(220, 0.07, "sine", 0.15 * v, 0, 120); },
    catch: (v) => { tone(500, 0.06, "triangle", 0.14 * v, 0, 800); },
    goal: (v) => { noise(1.2, 0.25 * v, 1200, "bandpass", 0.5); [523, 659, 784, 1047, 784, 1047].forEach((f, i) => tone(f, 0.22, "square", 0.1 * v, i * 0.09)); },
    whistle: (v) => { tone(2400, 0.25, "sine", 0.12 * v); tone(2400, 0.45, "sine", 0.12 * v, 0.32); },
